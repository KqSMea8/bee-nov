...
var Inq=(function(siteFcnStr,rulesEngineDataStr,loadedMbuRuleDataFuns){var isInqInitialized=false;eval(siteFcnStr);var site=initSiteData();var one2ManyChatrouterDomain=site.oneToManyChatrouterDomain||"api.inq.com";var one2ManyChat=false;var isConversationActive=false;var JSLoggingDisabled=site.JSLoggingDisabled;try{if(JSLoggingDisabled){JSLoggingDisabled=!window.localStorage.getItem("enableJSLogging");}}catch(e){}
var dataReady=false;var custID=null;var persistentCustomerID=null;var geoData=null;var custIP=null;var win=self.parent;var doc=win.document;var domObserver;var initRule={};var openerData={};var isChatProgressInSDK=false;var KEY_ENGAGEMENT_ID_STRING="engagementID";var KEY_CUSTOMER_ID_STRING="customerID";var KEY_WINDOW_ID_STRING="windowId";var COOKIE_PC_NAME="inqPc";var COOKIE_COBROWSE="cobrowse";var DEFERRED_START_OF_MANAGERS_KEY="DEFERRED_START_OF_MANAGERS";var DEFAULT_INITIAL_TIMEOUT=120;resetObserver();(function initializeFacade(){window.getParentElByIDTFacade=function(id){return window.parent.document.getElementById(id);};window.getParentElsByTNTFacade=function(tname){return window.parent.document.getElementsByTagName(tname);}}());(function overRideDomainsIfAny(){var v3Config=window.parent.v3LanderConfig;if(v3Config){site.chatRouterVanityDomain=(v3Config.crVanityDomain||site.chatRouterVanityDomain).replace(/(^\w+:|^)\/\//,'');site.vanityDomainName=v3Config.vanityDomain||site.vanityDomainName;site.cobrowseURL=v3Config.cbVanityDomain||site.cobrowseURL;site.clientStaticUrl=v3Config.clientStaticUrl||site.clientStaticUrl;site.mediaServer=v3Config.mediaUrl||site.mediaServer;site.xformsVanityDomain=(v3Config.xformsUrl||site.xformsVanityDomain).replace(/(^\w+:|^)\/\//,'');site.psHosturlList=v3Config.psHostedFileUrl||site.psHosturlList;}}());function Base64(){var _PADCHAR="=",_ALPHA="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",_VERSION="1.0";function _getbyte64(s,i){var idx=_ALPHA.indexOf(s.charAt(i));if(idx===-1){throw"Cannot decode k6ji";}
return idx;}
...
frameItem.reqQueue.push(request);if(frameItem.loaded){this.sendRequest(frameItem);}}catch(e){if(window.console)window.console.error(e.message,e);logErrorInPostToTagServer("LR45.postRequestToIframeProxy: "+e.message+". ["+(CHM.getChatID()?"chatID="+CHM.getChatID():"customerID="+Inq.getCustID())+", siteID="+Inq.getSiteID()+"]");}};LR45.prototype.setIFrameTimeout=function(key,iframe,p2pPath){if(p2pPath){LR45.pendingIFrames[key]=setTimeout(function(){this.refreshIFrame(key,iframe.src);}.bind(this),LR45.REFRESH_IFRAME_DELAY_INCREMENT);}else{setTimeout(function(){LR45.errorHandler(key);},LR45.REFRESH_IFRAME_DELAY_INCREMENT);}};LR45.prototype.refreshIFrame=function(key,src,delay){delay=delay||LR45.REFRESH_IFRAME_DELAY_INCREMENT;if(LR45.pendingIFrames[key]&&LR45.frameStorage[key]){if(delay<LR45.MAX_REFRESH_IFRAME_DELAY){delay+=LR45.REFRESH_IFRAME_DELAY_INCREMENT;}
var iframeItem=LR45.frameStorage[key];iframeItem.frame.contentWindow.location=src+"";LR45.pendingIFrames[key]=setTimeout(function(){this.refreshIFrame(key,src,delay);}.bind(this),delay);}};LR45.prototype.iFrameInitializationCallback=function(data){if(data.id){var iframeItem=LR45.frameStorage[data.id];if(LR45.pendingIFrames[data.id]){clearTimeout(LR45.pendingIFrames[data.id]);delete LR45.pendingIFrames[data.id];}
iframeItem.loaded=true;data.context.sendRequest(iframeItem);}};LR45.prototype.updateFrameName=function(id,url){var parsedUrl=parseUrl(url);var key=parsedUrl.origin;var frameItem=LR45.frameStorage[key];if(frameItem){var iframe=frameItem.frame;var name=frameItem.name;if(name.length>0){var cmnds=name.split("&&");var rem_indx=-1;for(var i=0;i<cmnds.length;i++){var cmd_id=cmnds[i].split("||")[1];if(cmd_id==id){rem_indx=i;break;}}
...
FcnMgr.prototype.ex8=function(fcnOrFcnName){if(!window.self.parent.v3Lander){logErrorToServer("window.self.parent.v3Lander is undefined, ex8 returns null");return null;}
...
if(retval==null)
retval={};retval[cookie_name]=cookie_value;});return retval;};CM47.prototype.setXDCookies=function(cookieData){this.cookies=cookieData;this.checkDataIntegrity();if(cookieData&&cookieData.pc&&cookieData.pc==2){this.isLSused=true;}};CM47.prototype.checkDataIntegrity=function(){var cacheCookiesInParentWin=openerCall("getCacheCookies");if(isFF()&&cacheCookiesInParentWin){var cacheCookiesKeys=Object.keys(cacheCookiesInParentWin);for(var i=0;i<cacheCookiesKeys.length;i++){if(!this.cookies[cacheCookiesKeys[i]]){this.cookies[cacheCookiesKeys[i]]=cacheCookiesInParentWin[cacheCookiesKeys[i]];}}}};CM47.prototype.onThirdPartyCookieQueueEmpty=null;CM47.prototype.onThirdPartyCookieQueueEmptyHandlers=[];CM47.prototype.on3rdPartyCookieCommittedHandler=null;CM47.prototype.isThirdPartyCookieQueueEmpty=function(){return(0==this.set3rdPartyCookieQueue.length)&&!this.set3rdPartyCookieBusy;};CM47.prototype.s3d=function(cmd,site,name,delta,path,expiry,secureCookie){if(!path)path="/";if(!expiry)expiry="";if(inu1(cmd)||inu1(site)||inu1(name))return;var boxID="box"+Math.floor(Math.random()*1000011);var port=(inqFrame.location.port!="")?":"+inqFrame.location.port:"";var parentURL=inqFrame.location.protocol+"//"+inqFrame.location.hostname+port+inqFrame.location.pathname;var sDelta="";if(!inu1(delta)){if(typeof delta==="object"){sDelta=encodeURIComponent(this.JSON.stringify(delta));}else{sDelta=""+delta;}}
var postCookie=[cmd,boxID,site,parentURL,name,sDelta,path,expiry,secureCookie];if(0==this.set3rdPartyCookieQueue.length)
...
setParam(currentCookiesObject,"inqVital.INQ.custID",CM47.chatSessionHelper.customerId);var sessionID=CM47.chatSessionHelper.customerId+"1";setParam(currentCookiesObject,"inqVital._iID",sessionID);setParam(currentCookiesObject,"inqSession._ssID",sessionID);setParam(currentCookiesObject,"inqSession.CHM.chat.id",engagement.engagementID);setParam(currentCookiesObject,"inqSession.chat.id",engagement.engagementID);setParam(currentCookiesObject,"inqSession._ecID",engagement.engagementID);setParam(currentCookiesObject,"inqSession._icID",engagement.engagementID);setParam(currentCookiesObject,"inqSession.chat.buID",engagement.businessUnits[0].businessUnitID);setParam(currentCookiesObject,"inqVital.CHM.lastChat.businessUnitID",engagement.businessUnits[0].businessUnitID);var brID=parseInt(engagement.businessRuleID);setParam(currentCookiesObject,"inqSession.CHM.chat.ruleID",brID);setParam(currentCookiesObject,"inqSession.chat.ruleID",brID);setParam(currentCookiesObject,"inqVital.CHM.lastChat.brID",brID);var rule=BRM.rb41(brID);setParam(currentCookiesObject,"inqSession.chat.chatType",rule?rule.ruleType:"C2C");if(engagement.agentGroups){setParam(currentCookiesObject,"inqSession.chat.agID",engagement.agentGroups[0].agentGroupID);setParam(currentCookiesObject,"inqVital.CHM.lastChat.agentGroupID",engagement.agentGroups[0].agentGroupID);setParam(currentCookiesObject,"inqSession.chat.agName",engagement.agentGroups[0].agentGroupName);setParam(currentCookiesObject,"inqVital.CHM.lastChat.agentGroupName",engagement.agentGroups[0].agentGroupName);}
...
return engagement;},isChatInProgress:function(){return(CM47.chatSessionHelper.isEnabled===true&&CM47.chatSessionHelper.isPersistentChat);}};function CCD(id,name,path,lifetime,domain,overflowPriority,cm){this._resource(id);this._observable();this.getName=function(){return name;};this.getPath=function(){return path;};this.getLifetime=function(){return lifetime;};this.getDomain=function(){return domain;};this.isSecure=function(){return site.secureCookie&&document.location.protocol=="https:"};this.getSecure=function(){return this.isSecure();};this.getPurgePriority=function(){return overflowPriority;};this.cm=cm;}
...
rom.ch.remove(channel);});channel.send(url,data,null,resendOnError,maxResendTimes,timeout);this.ch.push(channel);}catch(e){let12("RM91.send >> making HTTP Request. URL: "+url+cf21(e));}};this.post=function(url,data,id,sync,callbackFcn,headers){if(JSLoggingDisabled&&url===urls.loggingURL){return;}
...
try{this.sessionVar.z3(sessionData);}catch(e){log("GR34.save sessionData: "+e,e);}};GR34.prototype.isListener=function(l){if(l&&(l.onChatRequested||l.onChatLaunched||l.onChatShown||l.onChatClosed||l.onPersistentPush||l.onAgentMsg||l.onCustomerMsg||l.onAgentAssigned||l.onChatEngagedEvent||l.onChatEvent||l.onBeforeChatClosed||l.onAgentDataPass||l.onReceiveVADataPass||l.onQeueMsg))
...
if(inu1(this.getChat().getRule())){logErrorToServer("Rule for BU not found. Chat lost.");throw"Rule for BU not found. Chat lost.";}}
...
function getCacheCookies(){return CM.cookies;}
...
function SM19(surveyDataMap){var surveyURL=urls.baseURL+"/survey/genericSurvey.do";var NATIVE_SURVEY_EVENT="LoadPostChatSurvey";this.nativePostChatSurveyCall=null;this.eventData=null;this._observable();this.defaultWindowSpec="statusbar=0, scrollbars=1, resizable=1, toolbar=0, menubar=0, copyhistory=0";this.poppedWindows={};function getAltSurveyURL(data,evt){try{var brName=BRM.rb41(CHM.getLastChat().brID).getName();}catch(err){brName="NO BR Name Available";}
...
function isNonAsyncChatOrValidAsyncSurvey(chat,eventData){return!chat||!chat.isAsyncChat()||(eventData&&eventData.conversationResolved&&!eventData.pushNotificationDeviceID);}
...
Inq.PM.resetTmpVars();Inq.unblockService("ALL");LDM.start();}catch(e){let12("Error on reinitChatV3Callback call."+cf21(e));v3Lander.reInitChatData=null;}}
...
return true;}
function stop_CBC(){var cbc=FP.getCBC();if(cbc){cbc.stop();}else{logErrorToServer("CBC is not defined on FP.getCBC().stop call.");}}
function toggleMenuBoxForCobBanner_CBC(){var cbc=FP.getCBC();if(cbc){cbc.toggleMenuBoxForCobBanner();}else{logErrorToServer("CBC is not defined on FP.getCBC().toggleMenuBoxForCobBanner call");}}
function toggleTermsAndConditionDivForCobBanner_CBC(){var cbc=FP.getCBC();if(cbc){cbc.toggleTermsAndConditionDivForCobBanner();}else{logErrorToServer("CBC is not defined on FP.getCBC().toggleTermsAndConditionDivForCobBanner call");}}
function getTermsAndConditions_CBC(flag){var cbc=FP.getCBC();if(cbc){cbc.termsAndConditions(flag);}else{logErrorToServer("CBC is not defined on FP.getCBC().termsAndConditions call");}}
...
if(!frameworkCanRun(getDeviceType(),_3pcSupported,true,_xd)){if(document.cookie.indexOf("_inqNR=0")<0){lmt12(plc1("Framework load aborted: deviceType="
+getDeviceType()+";3pc="+_3pcSupported+";userAgent="+navigator.userAgent),LOG_LEVELS.WARN);document.cookie="_inqNR=0";}
return;}
var inqID=this.getID();if(initData[inqID]){this.setFrameworkData(initData[inqID]);}else if(!inu1(initData["siteTzOffset"])){log("Received site timezone offset from server: "+initData["siteTzOffset"]+"ms");this.setFrameworkData(initData);}else{lmt12(plc1("Framework load aborted due to incorrect initData: deviceType="
...
function checkAdBlockersActive(){if(!top.blockingDetector){CHM.setAdblockOn();}else{var onDetectedFn=CHM.setAdblockOn.bind(CHM);var onNotDetectedFn=CHM.setAdblockOff.bind(CHM);top.blockingDetector.onDetected(onDetectedFn).onNotDetected(onNotDetectedFn);top.blockingDetector.check(false);}}
