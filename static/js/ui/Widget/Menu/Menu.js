                    ...
                    '<% for(var i= 0, len= items.length, item; i<len; i++){ %>',
                        ...
                        '<% } else { %>',
                            '<li class="lbf-menu-item <%== item.disabled ? \'lbf-menu-item-disable\' : \'\' %>">',
                                '<a class="lbf-menu-label <%== item.items && item.items.length > 0 ? \'lbf-menu-label-hasSubMenu\' : \'\' %>" title="<%== item.title || \'\' %>" target="<%== item.target || \'_self\' %>" href="<%== item.href || \'javascript:;\' %>" data-level="<%== level %>" data-index="<%== i %>" data-value="<%== item.value %>"><%== item.content %></a>',
                            '</li>',
                        '<% } %>',
                    '<% } %>',
                '</ul>'
